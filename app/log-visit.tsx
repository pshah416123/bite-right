...
export default function LogVisitScreen() {
  ...
  const handleSave = async () => {
    ...
    const dishHighlight = dishes.length > 0 ? dishes[0] : undefined;
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.section}>
          ...
          {(loading || suggestions.length > 0) && !autocompleteError && (
                    ...
                    onPress={async () => {
                      if (s.placeId.startsWith('mock_')) {
                        ...
                        if (rest) {
                          ...
                          setRestaurantQuery(rest.name);
                          setSuggestions([]);
                        }
                        return;
                      }
                      try {
                        const restaurant = await selectRestaurant(s.placeId);
